...
It originated as a separate project to serve as a layer between the transient data used by the software framework and the data stored permanently, persistent.
The transient/persistent style of representing event data will be further explained in $\S$ \ref{sec:IO_EDM}.
...
Athena is hosted on GitLab and developed using CI with an instance of Jenkins, called ATLAS Robot, to build and test the new changes within a merge request interface. 
...
More information on how the xAOD EDM is deployed into unit tests in $\S$ \ref{sec:Mod_utests_xAOD_object}.
