...
The HL-LHC era will start sooner than that, and it's projected to demand anywhere from 6-10 times data stored per year, so any attempt to save on disk storage should be investigated.\cite{ATLAS_HL-LHC_projections}
...
One avenue optimziation is being investigated is in the method of storing data to file.
The traditional method of storing event information for AOD/DAOD is with ROOT TTrees (see Section \ref{section: ATLASIO_TTreeObject}).
