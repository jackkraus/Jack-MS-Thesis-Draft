...
Alternatively, the script can be run with \verb|root| and the \verb|ROOT| interpreter.
...
When the number of events is increased from $N=10^5$ to $N=10^6$, branches where only half of the mixture is random data become larger and the branches with more vectors per entry become more difficult to compress. 
...
As a result for these smaller baskets, ROOT creates baskets of at least this size, as it can only store one basket per vector.
